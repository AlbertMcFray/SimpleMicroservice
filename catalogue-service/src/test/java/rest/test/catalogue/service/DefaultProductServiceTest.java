...
class DefaultProductServiceTest {
    ...
    void findProduct_ProductExists_ReturnsNotEmptyOptional() {
        ...
        var productDTO = ProductUtil.convertToDto(product);
        ...
        assertEquals(productDTO, result.orElseThrow());
    ...
    void createProduct_ReturnsCreatedProduct() {
        // given
        var title = "New Product №1";
        var details = "New Description №1";

        doReturn(new Product(1, "New Product №1", "New Description №1"))
                .when(this.productRepository).save(new Product(null, "New Product №1", "New Description №1"));

        // when
        var result = this.service.createProduct(title, details);

        // then
        assertEquals(new Product(1, "New Product №1", "New Description №1"), result);

        verify(this.productRepository).save(new Product(null, "New Product №1", "New Description №1"));
        verifyNoMoreInteractions(this.productRepository);
    }
    ...
    void updateProduct_ProductExists_UpdatesProduct() {
        // given
        var productId = 1;
        var product = new Product(1, "New Product №1", "New Description №1");
        var title = "New Product №1";
        var details = "New Description №1";
